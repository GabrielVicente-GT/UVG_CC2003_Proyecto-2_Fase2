...
class App:
    ...
    def create_relationship(self, person1_name, game1_name, Type_name, Device_name, Company_name, Modo_name, Conexion_name):
        with self.driver.session() as session:
            ...
            for row in result:
                print("Creacion de relacion exitosa entre: {p1}, {g1}".format(p1=row['p1'], g1=row['g1'])+" y sus caracteristicas")
    # Creación de relación
    ...
    def _create_and_return_relationship(tx, person1_name, game1_name, Type_name, Device_name, Company_name, Modo_name, Conexion_name):
        query = (
            "MATCH (t1:Type { name: %s }), (d1:Device { name: %s }), (co1:Conection { name: %s }), (m1:Modo { name: %s })  "
    ...
    def delete_relationship(self, person1_name):
        with self.driver.session() as session:
            ...
            for row in result:
                print("Eliminación de recomendaciones por el usuario y de relaciones exitosa")
    ...
    def find_persons(self):
      with self.driver.session() as session:
        ...
        for row in result:
          print("Personas que recomendaron algun videojuego y se agrego a la base de datos: {row}".format(row=row))
...
if __name__ == "__main__":
    ...
    if MenuInicio == 1:
      ...
      try:
        print()
        print('Deberas de seleccionar las categorias de tu interes para esta opcion (2 maximo)')
        print('------------------------------------------------------------')
        ...
        try:
            ...
            try:
                ...
                print('2) Multiplayer')
                print()
                disp = int(input('Modalidad del videojuego (Multiplayer o Singleplayer) (1): '))
                if disp == 1:
                    Modo = 'Singleplayer'
                elif disp == 2:
                    Modo = 'Multiplayer'
            ...
            try:
                ...
                print('2) Conexion Offline')
                print()
                disp = int(input('Conexion del videojuego (Online u Offline) (1): '))
                if disp == 1:
                    Conexion = 'Online'
                elif disp == 2:
                    Conexion = 'Offline'
                ...
                VerificadorInicio = False
            print()
            app.create_relationship(Apodo, Game, Type, Device, Company, Modo, Conexion)
            print()
        except:
            print('La opción que ingresó no existe')
            print()
